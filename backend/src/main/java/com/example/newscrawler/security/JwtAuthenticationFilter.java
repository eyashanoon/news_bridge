    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            if (jwt != null && jwtTokenProvider.validateToken(jwt)) {
                ...
                // Block suspended accounts — but only if they don't have an active editor account
                if (email != null && !email.isEmpty()) {
                    // First, check if the user is an active editor (editor accounts take precedence)
                    EditorUser editor = editorUserRepository.findByEmail(email).orElse(null);
                    if (editor != null) {
                        if (editor.getStatus() == UserStatus.SUSPENDED) {
                            response.setStatus(HttpStatus.FORBIDDEN.value());
                            response.getWriter().write("{\"error\":\"Account is suspended\"}");
                            return;
                        }
                        // If editor is active, don't block even if registered user is suspended
                    } else {
                        // No editor account found — check regular admin/registered user
