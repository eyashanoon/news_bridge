...
let presenterAnimation = null; // default hand animation saved from Lip Sync Lab
...
loadCharacter(
  ...
  async () => {
    ...
    presenterAnimation = await fetchPresenterAnimation();
    characterReady = true;
...
async function handleSpeak(text) {
  ...
  try {
    ...
    if (enhancementEnabled && directives) {
      scheduleGestures(_gestureDurationMs, allPoses, directives.gesture);
    } else if (hasPresenterAnimation(presenterAnimation)) {
      schedulePresenterAnimation(_gestureDurationMs, allPoses, presenterAnimation);
    } else {
      scheduleGestures(_gestureDurationMs, allPoses, { enabled: false });
    }
