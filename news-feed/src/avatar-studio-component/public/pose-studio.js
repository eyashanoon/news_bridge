...
// loadPoses(part): restore saved poses from localStorage for one part.
// This keeps the avatar pose studio working without a backend /api/poses service.
export async function loadPoses(part) {
  try {
    ...
    allPoses[part] = {}; // localStorage unavailable or invalid JSON → empty
  }
}

// savePoses(part): persist the current in-memory pose map for one part to localStorage.
// Called after every save or delete in the Pose Studio.
async function savePoses(part) {
  try {
    localStorage.setItem(`avatar-studio-poses-${part}`, JSON.stringify(allPoses[part]));
  } catch {
    // Ignore localStorage errors so the UI remains usable.
  }
}
