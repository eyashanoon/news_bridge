...
// ── Manual panel: Save / Load ────────────────────────────────
const MA_STORAGE_KEY = 'av_manual_anim';

// Enable Load button if a saved config already exists
if (localStorage.getItem(MA_STORAGE_KEY)) maLoad.disabled = false;

maSave.addEventListener('click', () => {
  localStorage.setItem(MA_STORAGE_KEY, JSON.stringify(manualConfig));
  maLoad.disabled = false;
  // Brief visual feedback
  const orig = maSave.textContent;
  maSave.textContent = '✓ Saved';
  setTimeout(() => { maSave.textContent = orig; }, 1200);
});

maLoad.addEventListener('click', () => {
  ...
  try {
    ...
    maLoad.textContent = '✓ Loaded';
    setTimeout(() => { maLoad.textContent = orig; }, 1200);
  } catch (e) {
    console.warn('[lab] Failed to load saved animation config', e);
  }
});
...
async function loadPoses(part) {
  try {
    const saved = localStorage.getItem(`avatar-studio-poses-${part}`);
    allPoses[part] = saved ? JSON.parse(saved) : {};
  } catch {
    allPoses[part] = {};
  }
}
...
async function savePoses(part) {
  try {
    localStorage.setItem(`avatar-studio-poses-${part}`, JSON.stringify(allPoses[part]));
  } catch {
...
async function doGenerate(text) {
  ...
  try {
    ...
    // 3. Rhubarb
    setStatus('Running Rhubarb phoneme analysis…');
    const wavBytes = await _resampleToMonoWav(decodedBuffer);
    ...
    if (!res.ok) {
      ...
      throw new Error(`Rhubarb: ${err.error ?? `HTTP ${res.status}`}`);
    }
    const json = await res.json();
    rawCues = json.mouthCues ?? [];
    sections = buildSectionsFromRawCues();  // slice audio into per-section buffers
