...
const LATIN_STOPWORDS = {
  es: [" el ", " la ", " los ", " las ", " del ", " que ", " por ", " con ", " una ", " para ", " como ", " más ", " pero ", " también ", " este ", " esta ", " son ", " fueron ", " sobre ", " entre "],
  ...
  it: [" il ", " lo ", " la ", " le ", " gli ", " che ", " per ", " con ", " una ", " sono ", " del ", " della ", " anche ", " come ", " più ", " ma ", " questo ", " questa ", " stato ", " stata "],
  pt: [" o ", " a ", " os ", " as ", " do ", " da ", " que ", " para ", " com ", " uma ", " um ", " não ", " mais ", " como ", " por ", " também ", " seu ", " sua ", " são ", " foi "],
  nl: [" het ", " een ", " van ", " dat ", " die ", " voor ", " met ", " op ", " zijn ", " ook ", " naar ", " maar ", " niet ", " deze ", " dit ", " als ", " bij "],
...
/** Detect language from paragraph text (no metadata). Never defaults to English. */
...
/**
 * Detect effective language of a post or comment (ISO 639-1 two-letter code).
 * Content analysis overrides incorrect stored metadata. Never defaults to English.
 */
export function detectItemLanguage(item, extraContent = "") {
  ...
  // Only trust stored language when it's specific (not the generic en/ar default from ingestion)
  if (serverLang && serverLang !== "en") return serverLang;
  if (fieldLang && fieldLang !== "en") return fieldLang;
  return "";
}

/** Build a text sample from loaded article paragraph blocks. */
...
/** True when post language differs from the user's selected UI language. */
...
/** Target language for on-demand translation (user's UI language). */
...
/** Human-readable label for a detected language code (for post badges). */
